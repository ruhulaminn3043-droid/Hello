// We execute immediately. No event listener needed since the HTML loads first.
const wordsList = [
...
// Failsafe: Only run the animation if the container actually exists in the HTML
if (container) {
    ...
    for (let i = 0; i < totalWords; i++) {
        ...
        // Randomize the animation delay so they float independently like gas particles
    ...
    setTimeout(() => {
        wrappers.forEach(item => {
            ...
            // Massive random spread in 3D space
    ...
    setTimeout(() => {
        // Calm down the gas particle floating
...
function typeWriter() {
    if (charIndex < message.length) {
        ...
        // Failsafe check for the text container
        if (textContainer) {
            if (char === '\n') {
        ...
        // Show the pull-to-refresh hint when typing finishes (with failsafe)
